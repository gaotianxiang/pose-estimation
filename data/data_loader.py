...
#         features = tf.io.parse_example(example, features=image_feature_description)
...
def create_dataset(tfrecords, batch_size, num_heatmap=16, is_train=False):
    # preprocess = Preprocessor(heatmap_shape=(64, 64, num_heatmap), is_train=is_train)
...
if __name__ == '__main__':
    path = '/home/tianxiang/dataset/mpii/tfrecords_mpii'
    tfrecords = [os.path.join(path, fn) for fn in os.listdir(path) if fn.startswith('train')]

    train = create_dataset(tfrecords, 128, 16, True)
    ...
    print(type(image))
    plt.imshow(image)
    plt.imsave('./output.png', image.numpy())
    ...
    #     m = h1[:, :, i]
    #     plt.imshow(m)
    #     plt.imsave('./heatmap{}.png'.format(i), m.numpy())

    from visualize import draw_keypoints_on_image, draw_skeleton_on_image, extract_keypoints_from_heatmap

    kp = extract_keypoints_from_heatmap(h1)
    draw_keypoints_on_image(image, kp)
    draw_skeleton_on_image(image, kp)
